...
```
feat|fix|docs|style|refactor|test|chore: [AI-Q] フェーズ完了の概要
...
```
fix: [AI-Q] 認証機能のセキュリティ改善

### 変更内容の詳細

#### ファイル単位の変更概要
- src/auth/service.ts: パスワードハッシュ化の強度を向上
- src/auth/middleware.ts: トークン検証の厳格化
- src/auth/controller.ts: 入力バリデーションの強化
- .env.example: セキュリティ設定の追加

### 変更の趣旨
認証機能のセキュリティ脆弱性を修正し、より安全な実装に改善

### 技術的な詳細
- パスワードハッシュ化のラウンド数を12から15に増加
- JWTトークンの有効期限を1時間から30分に短縮
- 入力バリデーションに正規表現パターンを追加
- セキュリティヘッダーの追加
```
